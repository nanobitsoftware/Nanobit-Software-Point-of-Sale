...
specific file that theyre in a the time.
 ...
 Code description:
 Main entry file for Nano Point of sale. Facilitates all the memory 
 setup and window controls for the first few needed things. Minimal for  reason, as everything 
 else is handed off.
...
int WINAPI WinMain(HINSTANCE hThisInstance, HINSTANCE hPrevInstance, LPSTR lpszArgument, int nFunsterStil)
{
	...
	//icon = LoadIcon(hThisInstance, MAKEINTRESOURCE(MAIN_ICON));
	...
	*/
	...
	if (setupwindow)
		...
		LOG("Hash.password: %s",hash.password);
		LOG("Hash.hash: %s",hash.hash);
		LOG("Hash.pw_salt: %s",hash.pw_salt);
		LOG("Hash.salt: %s", hash.salt);;
		//return 0;
	while (1)
