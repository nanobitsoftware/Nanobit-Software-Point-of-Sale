...
specific file that theyre in a the time.
 ...
 Code description:
 Generates some reports and sends them off to printer.c so that the printer can
 do some complex printing stuff that it does. Either way all reports go through here
 so they can be paginated and santized.
